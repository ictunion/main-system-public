...
# signature is same for every application
...
limit=10
echo "Calling join API "$limit" times..."

for (( i=0; i<$limit; i++ )); do
    email="${emails[$i]}"
    first_name="${first_names[$i]}"
    last_name="${last_names[$i]}"
    date_of_birth="${dates_of_births[$i]}"
...
workplace_limit=3
echo "Calling create workplace API "$workplace_limit" times..."
...
for (( i=0; i<$workplace_limit; i++ )); do
    name="${workplace_names[$i]}"
    email="${workplace_emails[$i]}"
    group_id="${workplace_group_ids[$i]}"

    PAYLOAD=$(jq -n \
        --arg name "$name" \
        --arg email "$email" \
        --arg keycloak_group_id "$group_id" \
        '{
            "name": $name,
            "email": $email,
